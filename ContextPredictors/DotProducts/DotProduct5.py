...
class DotProduct(DotProduct):
    ...
    def classify(self,sbin,X):
        cntxt0 = np.dot( (self.base[self.b_rev[sbin],0,:]-self.means)/self.stds,
                        (X-self.means)/self.stds)
        cntxt1 = np.dot( (self.base[self.b_rev[sbin],1,:]-self.means)/self.stds,
                         (X-self.means)/self.stds)
    ...
    def train(self, X,Y):
        ...
        self.stds = np.std(X[:,:-1],axis=0)
        self.stds[self.stds==0] = 1 # If it's 0, then it doesn't matter anyways
